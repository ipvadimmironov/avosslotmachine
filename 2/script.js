...
function rollAll() {
  ...
  let lastDeltasMin = Math.min(...lastDeltas);
  lastDeltas = lastDeltas.map(el=>el-lastDeltasMin);


  Promise

  // Activate each reel, must convert NodeList to Array for this with spread operator
  .all([...reelsList].map((reel, i) => roll(reel, i,lastDeltas[i])))

  // When all reels done animating (all promises solve)
  .then(deltas => {
    // add up indexes
    deltas.forEach((delta, i) => indexes[i] = (indexes[i] + delta) % num_icons);
    debugEl.textContent = (indexes.every(i=>i==indexes[0])) ? indexes.map(i => iconMap[i])[0] : "Подпишись на бусти, заебал"
